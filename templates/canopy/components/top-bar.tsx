...
// below the logo/search/assistant row by Canopy's own theme.css (see the
...
// every other theme; only the underline treatment is Canopy-specific CSS.
...
/**
 * Build the three DocsShell header slot contents for Canopy.
 ...
 *  - The search trigger is wrapped in `.fw-canopy-search-center` so it reads
...
export function buildTopBar(config: DocsConfig, activeTab: string): TopBarParts {
  ...
  const topActions = (
    <>
      <div className="fw-canopy-search-center">
        <SearchDialog />
      </div>
