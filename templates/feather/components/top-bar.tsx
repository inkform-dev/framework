...
import { ResourcesTab } from './resources-dropdown';
...
/**
 * Build the three DocsShell header slot contents for Feather.
 * Server component — client islands (SearchDialog, ThemeToggle, AskAi,
 * ResourcesTab) are already marked 'use client' in their own modules.
 */
export function buildTopBar(config: DocsConfig, activeTab: string): TopBarParts {
  ...
  const topNav = (
    <>
      ...
      {/* "Resources" — a UI-only dropdown tab, not part of docs.json's
          DocsTab model (see components/resources-dropdown.tsx for why).
          Rendered as a sibling of .fw-topnav-tabs, not inside it — that
          div has overflow-x:auto (layout.css, for horizontal tab
          scrolling on narrow viewports), which per the CSS spec computes
          overflow-y to auto too, clipping an absolutely-positioned
          dropdown panel that opens below the tab row. .fw-shell-topnav
          (this fragment's parent) has no overflow set, so the panel
          isn't clipped here. */}
      <ResourcesTab />
