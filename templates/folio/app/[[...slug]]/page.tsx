...
import { resolveRoute, listAllRoutes, sidebarForDoc, sidebarFooter, withContentNavLinks } from '@/lib/route';
...
export default async function Page({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  return (
    <DocsShell
      ...
      sidebar={<Sidebar groups={sidebar} footer={sidebarFooter(config)} />}
      toc={headings.length > 0 ? <TocList headings={headings} /> : undefined}
      hideToc={headings.length === 0}
    >
      {/* Willow-style eyebrow — the page's active sidebar group (e.g.
          "Essentials"), styled uppercase in theme.css. Falls back to the
          tab name for pages the eyebrow logic can't otherwise label. */}
      <p className="fw-eyebrow">{ref.group}</p>
