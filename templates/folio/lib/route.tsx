/**
 * lib/route.ts — Folio routing core.
 *
 ...
 *
 * Folio-specific addition: sidebarFooter() — Folio drops the header CTA
 * button entirely (see components/top-bar.tsx) in favor of an all-caps
 * "Dashboard" link pinned to the bottom of the sidebar, Willow-style. It
 * reuses the existing `cta` config field (docs.json) rather than inventing a
 * new one — same data, different slot, still fully configurable.
 */
...
/**
 * Sidebar-bottom "Dashboard" link (passed as <Sidebar footer={...}>). Folio
 * has no header CTA button — see components/top-bar.tsx, which no longer
 * renders config.cta at all. Returns null when no `cta` is configured, so a
 * project that deletes it from docs.json just gets no footer, not a broken
 * link.
 */
export function sidebarFooter(config: DocsConfig): ReactNode {
  if (!config.cta) return null;
  const external = config.cta.href.startsWith('http');
  return (
    <a
      href={config.cta.href}
      className="fw-sidebar-cta"
      {...(external ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
    >
      <span className="fw-sidebar-cta-label">{config.cta.label}</span>
      <span className="fw-sidebar-cta-ext" aria-hidden>
        ↗
      </span>
    </a>
  );
