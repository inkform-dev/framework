# Harbor — @inkform/theme-harbor

Harbor is a dark-mode-forward documentation theme built on
`@inkform/framework` — inspired by Mintlify's Almond starter kit. Teal
accent, Inter throughout (one confident sans voice, not a serif/sans split),
JetBrains Mono for code. It supports Guides + API Reference tabs out of the
box.

Three things set Harbor apart from this repo's other themes:

- **Search lives in the sidebar, not the header.** The search trigger is
  passed as `<Sidebar header={...}>` (see `components/top-bar.tsx`'s
  `sidebarHeader` and `app/[[...slug]]/page.tsx` /
  `app/api-reference/[[...slug]]/page.tsx`), not rendered in the header's
  `topActions` like every other theme.
- **A Login / Sign-up button pair instead of a single CTA.** `docs.json` has
  no field for two auth buttons, so `components/top-bar.tsx` hardcodes
  placeholder hrefs (`LOGIN_HREF` / `SIGNUP_HREF` — point them at your real
  auth routes).
- **A "Copy page" button next to each doc page's H1** —
  `components/copy-page-button.tsx`, a small client component that copies
  the page's raw Markdown to the clipboard.
...
- Harbor doesn't read a `cta` field (unlike other themes) — see the Login /
  Sign-up note above.

### Writing MDX

Add `.mdx` files under `content/docs/` and register them in `docs.json`. Harbor
...
4. **Root directory**: set to the path of this folder if deploying as a
   standalone (e.g. `templates/harbor`), or leave blank if deploying the
   directory itself.
