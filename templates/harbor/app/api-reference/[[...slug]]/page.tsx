...
// Harbor builds its own <Sidebar> here (via buildReferenceSidebarGroups)
// instead of using the framework's <ReferenceSidebar> convenience wrapper —
// ReferenceSidebar doesn't expose the underlying Sidebar's `header` prop, and
// Harbor needs that slot for the sidebar-embedded search trigger (same as
// app/[[...slug]]/page.tsx). buildReferenceSidebarGroups is the same public
// helper ReferenceSidebar itself calls, so this renders identically plus the
// header. See this build's report for why that wasn't fixed upstream instead.
