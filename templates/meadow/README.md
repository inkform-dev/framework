# Meadow — @inkform/theme-meadow

Meadow is a light, amber-toned documentation theme built on
`@inkform/framework` — inspired by Mintlify's Aspen starter kit. Light-mode
forward with a warm amber/gold accent, Plus Jakarta Sans throughout, and
JetBrains Mono for code. The homepage is a real landing page (hero + CTA +
feature grid), not just an "Introduction" doc page. It supports Guides + API
Reference tabs out of the box.
...
- A nav group's `icon` (like `compass` above) renders as a small icon before
  the group label in the sidebar — see `lib/icons.tsx` for the name → Lucide
  icon map; add an entry there if you reference a name that isn't mapped yet.

### Writing MDX

Add `.mdx` files under `content/docs/` and register them in `docs.json`. Meadow
...
The homepage (`content/docs/index.mdx`) builds its feature-card grid by
nesting plain JSX (a dashed placeholder box, title, description, and tag
pills) inside the shared `<Card>`/`<CardGroup>` — reusing their grid, border,
and hover behavior — styled with Meadow-local `fw-meadow-*` classes in
`app/theme.css`. Copy that pattern for your own feature sections.
...
4. **Root directory**: set to the path of this folder if deploying as a
   standalone (e.g. `templates/meadow`), or leave blank if deploying the
   directory itself.
