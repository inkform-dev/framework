# Rowan — @inkform/theme-rowan

Rowan is a minimal-chrome documentation theme built on `@inkform/framework`
— inspired by Mintlify's own Maple starter kit. Orange/amber accent, Inter
throughout (one confident sans voice, no serif face), JetBrains Mono for
code. Unlike most themes here, Rowan has **no Guides|API-Reference tab
switcher** — every page shares one flat sidebar mixing doc pages and API
operations, plus a "Copy page" action next to the title, a system/light/dark
icon toggle by the search bar, and a sidebar-bottom language switcher.
...
Rowan's `docs.json` still declares those same two `tabs` internally (Guides +
API Reference) even though the UI never renders a switcher between them —
see the doc comment on `buildTopBar()` in `components/top-bar.tsx` for why
(short version: the alternative single-implicit-tab config shape breaks the
API Reference route's URL base). You don't need to think about this to add
content — just add pages/groups under the "Guides" tab as shown above.

### Writing MDX

Add `.mdx` files under `content/docs/` and register them in `docs.json`.
Rowan supports all built-in blocks: `<Note>`, `<Tip>`, `<Warning>`, `<Card>`,
...
Every page — doc or API operation — renders the same sidebar, built by
`lib/route.tsx`'s `sidebarFlat()`: doc nav groups first, then the API tab's
tag groups (with method-colored pills), in one list, with whichever item is
current highlighted. There's no separate Guides/API-Reference view to
navigate between.

### Copy page

Every page has a "Copy page" button next to its title
(`components/copy-page-button.tsx`, Rowan-only — not part of the shared
framework) that copies the page's Markdown to the clipboard. Its chevron
opens a small menu with a "View as Markdown" fallback that opens the raw
text in a new tab.
...
4. **Root directory**: set to the path of this folder if deploying as a
   standalone (e.g. `templates/rowan`), or leave blank if deploying the
   directory itself.
