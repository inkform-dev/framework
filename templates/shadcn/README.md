# Shadcn — @inkform/theme-shadcn

Shadcn reproduces shadcn/ui's own default look on `@inkform/framework`: Zinc
grayscale surfaces, a monochrome primary that inverts between light and dark
(a near-black button on white, a near-white button on black — shadcn ships
no brand hue by default), 0.5rem radius, and Tailwind's own soft shadow
scale. Structurally identical to Canopy (same header, sidebar, mobile
drawer, and search patterns — only the token layer differs): a centered
header search bar, underline tab indicators, no header CTA, a whole-sidebar
collapse toggle, and a sidebar-footer language + theme switcher. Inter
throughout, JetBrains Mono for code. It supports Guides + API Reference tabs
out of the box.
...
- Shadcn's header has no CTA button and no extra anchor/nav links by design
...
Add `.mdx` files under `content/docs/` and register them in `docs.json`. Shadcn
...
`components/collapsible-sidebar.tsx` is Shadcn-specific (not part of the
...
4. **Root directory**: set to the path of this folder if deploying as a
   standalone (e.g. `templates/shadcn`), or leave blank if deploying the
   directory itself.
