...
// below the logo/search/assistant row by Shadcn's own theme.css (see the
...
// every other theme; only the underline treatment is Shadcn-specific CSS.
...
/**
 * Build the three DocsShell header slot contents for Shadcn.
 ...
 *  - The search trigger is wrapped in `.fw-shadcn-search-center` so it reads
...
export function buildTopBar(config: DocsConfig, activeTab: string): TopBarParts {
  ...
  const topActions = (
    <>
      <div className="fw-shadcn-search-center">
        <SearchDialog />
      </div>
