# Yucca — @inkform/theme-yucca

Yucca is a dark-mode-forward documentation theme built on
...
- Yucca's header has no CTA button and no extra anchor/nav links by design
...
Add `.mdx` files under `content/docs/` and register them in `docs.json`. Yucca
...
`components/collapsible-sidebar.tsx` is Yucca-specific (not part of the
...
4. **Root directory**: set to the path of this folder if deploying as a
   standalone (e.g. `templates/yucca`), or leave blank if deploying the
   directory itself.
